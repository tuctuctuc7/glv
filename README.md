...
- Influ code revenue comes directly from `Revenue INFLU ($)`; no-code revenue is total revenue minus code revenue. Split children show Revenue, Avg daily revenue and monthly revenue Share only.
- Phases-only Avg daily revenue (USD) is revenue divided by distinct represented CZSK phase dates in the month, including zero-revenue days and excluding missing/future dates. Duplicate source dates still fail validation; aggregation counts distinct dates. Cross-month parents use summed revenue divided by all eligible dates, not a mean of monthly averages. Day rows divide by one; absent inputs or days produce null. Code/no-code averages use the same parent Influ-day denominator, including zero-code days, and reconcile to the parent. Both Revenue and Avg daily revenue support split chart curves; unsupported metrics retain Influ total. Revenue remains the default; the new column and option follow Revenue without changing Home.
- Share is always a month-local denominator: row revenue divided by total CZSK revenue for that same month.

Phases UI refinement (`nc-rc-metrics-20260920` asset version): high-level Phases multiselect defaults to all, and the default-off Influ split controls chart and table together. Revenue replaces Influ with Code and No code lines; unsupported chart metrics retain the total with disclosure. Table-local Months defaults to all. Phase-first split drilldown is Phase → Code/No code → Month → Days. Monthly Share denominators remain full CZSK months before phase filtering; cross-month code groups use full selected-month CZSK revenue. The table has a 520px internal scroller with pinned headers, compact desktop rows, and 44px mobile disclosures. Only the bottom-left chart legend is shown.

`BLENDED` rows are excluded. The dashboard `All` filter is calculated from `CZSK`, `US`, and `ROW` rows.

Promo comparison sits below Phase detail with independent Months and Metric controls. It compares daily CZSK Promo values on Day 1…N, concatenating represented Promo dates within each month. Defaults select the latest two months with actual Promo data, including late-arriving data when no prior selection existed; explicit None/All/custom choices persist across rerenders. Shorter months have null tails, not manufactured zeros. Calendar-month colors are stable across selections with light/dark palettes. Tooltips and accessible data include actual dates. NC ROAS and RC revenue are available alongside the other daily metrics. Maintained browser QA covers empty-to-populated recovery, explicit None, outside-click closure and open mobile menus.

Derived metrics are calculated after date and region aggregation:

- NC ROAS = NC revenue / existing Spend (ad spend, the same denominator scope as ROAS). Sum matching available NC/spend rows first; nonpositive summed spend is unavailable.
- RC revenue = revenue minus NC revenue, summed only over rows with both inputs. Explicit zero and negative residuals are preserved; negative residuals flag source discrepancies, not a clamped estimate.
- Partial inputs report an available subset, analogous to Home availability-aware metrics. Historical 2025 NC is unknown: historical-only new metrics remain unavailable and mixed periods never subtract current NC from all-history revenue.
- Included surfaces: both Home chart selectors, chart series/tooltips/accessible data, Audit summary and detail, market comparison and total, working-source CSV columns, Phases metric dropdown/chart/accessible data and all supported hierarchy aggregates/day rows. CAC stays immediately after ROAS, NC ROAS follows CAC; RC revenue follows NC revenue. Home NC rate remains after AOV; Phases days stays last and Avg daily revenue stays zero-decimal.
- Excluded deliberately: the existing eight executive cards remain unchanged; no ninth/tenth card. CSV remains working-source raw market-day scope (no historical rows), with derived columns appended. Code/no-code descendants have neither NC nor cost attribution, so both metrics remain null; the split chart retains disclosed Influ total. Named-influencer and Meta dashboards are outside this business-dashboard scope and unchanged; no invented allocations. Source JSON, exporter, existing formulas and auth/routes are unchanged.
- ROAS = revenue / spend
